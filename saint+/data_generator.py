            ...
            if len(item_id) > self.seq_len:
                ...
                if initial > 2:
                    ...
                    self.samples[f"{user_id}_0"] = (
                        ...
                        elaps_ed[:initial], assessmentItemAverage[:initial], UserAverage[:initial],
                        answer_code[:initial]
                    )
                ...
                for c in range(chunks):
                    ...
                    self.samples[f"{user_id}_{c+1}"] = (
                        ...
                        elaps_ed[start:end],assessmentItemAverage[start:end], UserAverage[start:end],
...
class Riiid_Sequence2(Dataset):
        ...
        for user_id in groups.index:
            ...
            if len(item_id) > self.seq_len:
                ...
                self.samples[f"{user_id}"] = (
                    ...
                    UserAverage[-seq_len:], answer_code[-seq_len:])
            else:
                self.user_ids.append(f"{user_id}")
                self.samples[f"{user_id}"] = (item_id,test_id,t_lag,elaps_ed,assessmentItemAverage,UserAverage,answer_code)
    def __len__(self):
    ...
    def __getitem__(self, index):
        ...
        label = np.clip(label, 0, 1)
        answercode = answercode[:-1]
        return itemid, time_lag, elapsed, itemaver, useraver, answercode, 