...
class ICECREAM(Dataset):
    def __init__(self, max_seq_len, config=None, dataset_dir=DATASET_DIR) -> None:
        ...
        self.q_seqs, self.r_seqs, self.q_list, self.u_list, self.r_list, self.q2idx, self.u2idx = self.preprocess()

        self.num_u = self.u_list.shape[0]
        self.num_q = self.q_list.shape[0]
        self.num_r = self.r_list.shape[0]

        self.q_seqs, self.r_seqs, train_add, test_add = \
    ...
    def match_seq_len(self, q_seqs, r_seqs, max_seq_len, pad_val=-1):
        ...
        for idx, (q_seq, r_seq) in enumerate(zip(q_seqs, r_seqs)):
        ...
        return proc_q_seqs, proc_r_seqs, train_add, test_add